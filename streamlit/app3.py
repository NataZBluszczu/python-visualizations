...
# 3. PANEL BOCZNY (Sidebar z Expanderami)
...
with st.sidebar.expander("1. Wybór Zmiennych", expanded=True):
    ...
    # Zabezpieczenie, jeśli mamy tylko jedną kolumnę numeryczną
...
with tab_viz:
    ...
    # Tworzenie odpowiedniego wykresu w Plotly
    ...
    # Personalizacja siatki (na podstawie checkboxa)
    if fig:
...
# --- ZAKŁADKA 2: STATYSTYKA ---
with tab_stat:
    st.subheader("Podsumowanie dla wybranych zmiennych numerycznych")
    # Zastosowanie df.describe() zgodnie z wymogiem zadania
    st.dataframe(df[[val_x, val_y]].describe(), use_container_width=True)
    
    # Dodatkowy bajer: Macierz korelacji w rozwijanym panelu
